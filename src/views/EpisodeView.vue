...
const selectedSource = ref(null) // Menyimpan kualitas yang dipilih
...
const fetchAnimeData = async (slug) => {
  ...
  try {
    ...
    epStream.value = responseA.data.server.qualities
    ...
    const responseB = await getAnimeResponse(`otakudesu/server/${selectedSource.value}`)
    ...
    if (responseB.data.length > 0) {
      ...
      if (!responseA.data.defaultStreamingUrl) {
        selectedSource.value = responseA.data.server.qualities[1].serverList[0].serverId // Gunakan sumber pertama dari kualitas pertama
      }
    }
    ...
    if (responseA.data.info.episodeList) {
      episodes.value = responseA.data.info.episodeList
        .map((episode) => {
          const episodeSlug = episode.episodeId // Ambil slug
          return {
            slug: episodeSlug,
            nomor: episode.title,
            judul: `Episode ${episode.title}` // Kustomisasi judul jika diperlukan
          }
        })
    }
...
const onSourceChange = async (source) => {
  const responseC = await getAnimeResponse(`otakudesu/server/${source}`)
  selectedSource.value = responseC.data.url
}
  ...
  <div v-else>
    ...
    <div class="grid xl:grid-cols-2 grid-cols-1 p-4 gap-4">
      ...
      <div>
        <iframe
          ...
          :src="selectedSource" frameborder="0"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
          allowfullscreen></iframe>
        <div v-if="epPrev || epNext"
      ...
      <div class="bg-color-dark rounded-lg p-4">
        ...
        <div class="mb-4">
          ...
          <select v-model="selectedSource" @change="onSourceChange(selectedSource)"
            class="w-full p-2 rounded bg-gray-800 text-white border border-gray-600">
            <option selected disabled>Pilih Kualitas</option>
            <optgroup v-for="quality in epStream" :key="quality.title" :label="quality.title">
              <option v-for="server in quality.serverList" :key="server.serverId" :value="server.serverId">
                {{ server.title }}
              </option>
            </optgroup>
          </select>
    ...
    <div class="bg-color-dark p-4 m-4 rounded-lg shadow-lg transition-all duration-500">
      ...
      <div class="grid grid-cols-1 gap-4 m-4 lg:grid-cols-3">
        <div v-for="download in downloads" :key="download.title"
          class="text-color-whity p-3 rounded-lg bg-color-primary">
          <p>{{ download.title }} - {{ download.size }}</p>
          <div class="flex flex-wrap gap-2 mt-2">
            <a v-for="url in download.urls" :key="url.title" :href="url.url" target="_blank"
    ...
    <div class="bg-color-dark p-4 m-4 rounded-lg shadow-lg transition-all duration-500">
      ...
      <div class="flex flex-col">
        <button @click="toggleComment" v-show="isComment === false"
      ...
      <div class="flex flex-col">
        <button @click="toggleComment" v-show="isComment === true"
          class="text-color-whity p-3 mx-4 rounded-lg bg-color-primary">
          Tutup Komentar
        </button>
      </div>
